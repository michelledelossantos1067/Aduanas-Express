...
function confirmar() {
    ...
    emit('confirmar', {
        ...
        // costo = null significa "sin cambio", el padre conserva el costo original
...
<template>
    <div v-if="modelValue" class="modal-overlay" @click.self="cerrar">
        <div class="modal-box-form">
            ...
            <div class="campo">
                <label>Reporte final <span class="opcional">(qué se encontró, qué se hizo además de lo previsto)</span></label>
...
.modal-titulo { font-size: 1.05rem; font-weight: 700; color: #111827; margin: 0 0 4px; }
.modal-desc { font-size: .82rem; color: #6b7280; margin: 0 0 16px; line-height: 1.4; }
.notif { padding: 10px 14px; border-radius: 8px; font-size: .8rem; margin-bottom: 14px; }
...
.campo label { font-size: .78rem; font-weight: 600; color: #374151; }
.req { color: #dc2626; }
.opcional { font-weight: 400; color: #9ca3af; }
.campo input, .campo textarea {
    padding: 9px 12px; background: #f9fafb; border: 1.5px solid #e5e7eb;
    border-radius: 8px; font-size: .875rem; color: #111827;
    font-family: inherit; outline: none; transition: border-color .15s;
}
.campo input:focus, .campo textarea:focus { border-color: #1a3a2a; background: #fff; }
.campo textarea { resize: vertical; }
.campo-checkbox { flex-direction: row; align-items: center; }
.check-label {
    display: flex; align-items: center; gap: 8px; cursor: pointer;
    font-size: .82rem !important; font-weight: 500 !important; color: #374151;
}
.check-label input[type="checkbox"] { width: 16px; height: 16px; cursor: pointer; }
.modal-acciones { display: flex; gap: 10px; margin-top: 6px; }
.btn-cancelar {
    flex: 1; padding: 10px 0; background: transparent; border: 1.5px solid #e5e7eb;
    border-radius: 9px; font-size: .875rem; font-weight: 500; color: #6b7280;
    cursor: pointer; transition: background .15s;
}
.btn-cancelar:hover:not(:disabled) { background: #f3f4f6; }
.btn-cancelar:disabled { opacity: .5; cursor: default; }
.btn-finalizar-confirmar {
    flex: 1.4; display: inline-flex; align-items: center; justify-content: center; gap: 7px;
    padding: 10px 0; background: #1a3a2a; border: none; border-radius: 9px;
    font-size: .875rem; font-weight: 700; color: #fff; cursor: pointer; transition: background .15s;
}
.btn-finalizar-confirmar:hover:not(:disabled) { background: #14532d; }
.btn-finalizar-confirmar:disabled { opacity: .6; cursor: default; }
.spinner-btn {
