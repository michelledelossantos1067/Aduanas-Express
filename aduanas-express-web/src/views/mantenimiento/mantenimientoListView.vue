...
async function confirmarEliminar(r) {
    ...
    try {
        ...
        // el error ya se notifica dentro del composable
    } finally {
...
<template>
    <div class="mant-page">
                        ...
                        <tbody>
                            <tr v-for="r in registrosFiltrados" :key="r.id">
                                ...
                                <td data-label="Vehículo">
                                    <button class="placa-badge placa-badge-btn" @click="abrirHistorial(r)" title="Ver historial de este vehículo">
                                        {{ r.vehiculoPlaca ?? '—' }}
                                    </button>
                                </td>
                                <td data-label="Tipo">
        ...
        <MantenimientoEliminarModal
            v-model="mostrarEliminarModal"
            :registro="registroAEliminar"
            @confirmar="confirmarEliminar"
        />

        <MantenimientoFinalizarModal
            v-model="mostrarFinalizarModal"
            :registro="registroAFinalizar"
            :guardando="finalizando"
            @confirmar="confirmarFinalizar"
        />

        <MantenimientoHistorialModal
            v-model="mostrarHistorialModal"
            :vehiculo-id="historialVehiculoId"
            :vehiculo-placa="historialVehiculoPlaca"
            :registros="registros"
        />
...
.placa-badge-btn:hover { background: #374151; }
...
@media (max-width: 760px) {
    ...
    .tabla-wrap    { background: transparent; box-shadow: none; overflow: visible; }
    ...
    .mant-tabla thead { display: none; }
    .mant-tabla tbody { display: block; }
    ...
    .mant-tabla tbody tr:last-child { margin-bottom: 0; }
