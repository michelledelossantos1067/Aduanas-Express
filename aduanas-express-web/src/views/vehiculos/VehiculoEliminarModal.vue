...
async function confirmar() {
    if (!props.vehiculo?.id) return
    loading.value = true
    error.value = ''
    try {
        await eliminarVehiculo(props.vehiculo.id)
        emit('eliminado', props.vehiculo.id)
        emit('update:modelValue', false)
    } catch (e) {
        ...
        <div v-if="modelValue" class="modal-overlay" @click.self="cancelar">
            <div class="modal">
                ...
                <p class="modal-desc">
                    ...
                    <strong v-if="vehiculo">{{ vehiculo.matricula }} — {{ vehiculo.marca }} {{ vehiculo.modelo }}</strong>.
